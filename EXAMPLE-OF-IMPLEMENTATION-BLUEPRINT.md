...
2.  **Resource Context**: 
    ...
    - 📚 **Crate Documentation**: Список путей в `docs.rs` или конкретных элементов (struct/trait), которые необходимо изучить через `tavily (search/extract)` перед написанием кода.
...
- **Новые зависимости:** Если фаза включает использование новой зависимости — включи шаг `tavily_search` и `tavily_extract` для проверки фич и версий.
- **Сложные Trait'ы:** Если фаза включает реализацию трейта (напр. `FromRequest`, `Serialize`, `Future`) — включи шаг чтения документации этого трейта через `tavily_extract`.
- **Breaking Changes:** Если используется популярный крейт (напр. `tokio`, `axum`, `serde`), всегда планируй проверку сигнатур методов через `tavily_search` или `tavily_extract`, так как они часто меняются.
...
**Steps**:
1. [ ] **Verify API**: Use `tavily_search` for `tokio_util::codec` to confirm `LinesCodec` vs custom `Decoder` traits.
2. [ ] **Validate Signatures**: Use `tavily_extract` for `TcpListener::accept` to check the returned `SocketAddr` handling in the current Tokio version.
...
[!IMPORTANT]
Before implementing the `Decoder` trait, use `tavily_extract` to retrieve the documentation for `tokio_util::codec::Decoder` to ensure `decode` and `decode_eof` signatures match the latest docs.rs.
...
**Steps**:
1. [ ] **Verify**: Use `tavily_search` to check `rust_decimal` docs for `checked` operations.
2. ...
